...
void update_branch_coverage(state_info_t *state) {
  ...
  for (int i = 0; i < MAP_SIZE; i++) {
    // \C8\E7\B9\FB\B5\B1ǰ\B7\D6֧\B1\BB\B4\A5\B7\A2
...
void update_state_branch_coverage(unsigned int *state_sequence, int state_count) {
  ...
  for (int i = 0; i < state_count; i++) {
    ...
    if (k != kh_end(khms_states)) {
      ...
      // \B8\FC\D0\C2״̬\B5ķ\D6֧\B8\B2\B8\C7ͼ
      ...
      switch(mode) {
        case FAVOR:
		  if(vanilla_afl){
            ...
            state->score = ceil(1000 * pow(2, -log10(log10(state->fuzzs + 1) * state->selected_times + 1)) * pow(2, log(state->paths_discovered + 1))* pow(2, 5 * log(rare_branch_count + 1)));
            break;
		  }
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "unique_hangs, max_depth, execs_per_sec, state_id_counts, prev_nodes, prev_edges\n");
                     /* ignore errors */
